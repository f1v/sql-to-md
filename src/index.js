...
(async () => {
    ...
    const dbSchemas = await Promise.all(userDbs.map(async (db) => {
        const tables = (await exec(`SHOW TABLES FROM ${db}`))[0].map(table => table[`Tables_in_${db}`]);
        return {
            db,
            ...
            WHERE
                ...
                AND REFERENCED_TABLE_NAME = ?`, [db, table]);
            ...
            WHERE table_schema = '${db}' AND table_name = '${table}'`, [db, table]);

                return {
                    table,
                    constraints: groupBy(constraints, 'CONSTRAINT_NAME'),
                    columns: columns.map((column) => {
                        const foreignKey = relationships.filter((relationship) => relationship.COLUMN_NAME === column.COLUMN_NAME);
                        const relatedBy = relationships.filter((relationship) => relationship.REFERENCED_COLUMN_NAME === column.COLUMN_NAME);
    ...
    console.log(JSON.stringify(dbSchemas, null, 2))
